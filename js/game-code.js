		...
		class Asteroid
		{
			...
			*/
			constructor()
			{
				...
				this.explosionImage = loadImage("./Assets/hit.png");
				this.load();
			}
		...
		class MissilePack
		{
			...
			constructor()
			{
				this.x = 0;
				this.y = 0;
				this.image = loadImage("./Assets/Chest.png");
		...
		class SpaceShip
		{
			...
			constructor()
			{
				this.image = loadImage("./Assets/Albjorf.png");
		...
		{	
			...
			*/
			constructor()
			{
				...
				{
					let live = loadImage("./Assets/hart.png");
					this.lives[i] = live;
				}
			}
		...
		*/
		function preload() 
		{
			background = loadImage("./Assets/Floor.png");		// load the background-image
			asteroidImage = loadImage("./Assets/Guard.png"); // load once and the pass to Asteroid so that will not load each time an Asteroid is created
			spaceship = new SpaceShip();
			
			missileImage = loadImage("./Assets/Dagger.png");
			missilePack = new MissilePack();
			
			gameSounds 
				$.get('./Pure/game-patch.pd', function(patchStr) {
		...
		function showMessages()
		{
			...
			fill(1,1,1);
			text("Daggers: " + spaceship.missiles, 1100, 60); // Score is shown
		    fill(1,1,1);
			if (!startGame || gameOver) // provide instructions
			{
				text('Press N to start a new game.', 340, 300, 800, 200);
				fill(1,1,1);
				textSize(25);
				text('Use the left and right arrows to avoid the guards.', 340, 360, 800, 200);
				fill(1,1,1);
				textSize(25);
				text('Get the daggers and throw by pressing the Space bar!', 340, 390, 800, 200);
				fill(1,1,1);
				textSize(25);
			}
